...
Object.assign(LexisUI.prototype, {
    ...
    onHFImagePicked(position, input) {
        ...
        reader.onload = (e) => {
            ...
            // Pojistka: odmítni poškozený/neúplný obrázek (ať v hlavičce nevznikne „duch").
            if (window.LexisLogoClean && window.LexisLogoClean.isDecodable) {
                window.LexisLogoClean.isDecodable(raw).then((ok) => {
                    if (!ok) { this.customAlert('⚠️ Obrázek je poškozený nebo neúplný a nelze ho vložit. Zkus jiný soubor (PNG/JPG/SVG).'); return; }
                    proceed();
                });
            } else { proceed(); }
        };
    ...
    insertFirmLogo(position) {
        ...
        const LOGO = (window.LEXIS_FIRM_LOGO || '');
        if (!LOGO) { this.customAlert('Logo kanceláře není k dispozici.'); return; }
        const apply = (dataUrl) => {
            this._hfImages[position] = dataUrl;
            const imgEl = document.getElementById(`hf-img-${position}`);
            if (imgEl) { imgEl.src = dataUrl; imgEl.style.display = 'block'; }
    ...
    applyHFTemplate(type) {
        if (type === 'firma') { this._applyHFFromProfile(); return; }
        const templates = {
            advokatura: {
    ...
    // Složí obsah editoru záhlaví z uloženého profilu kanceláře (logo vlevo,
    // identita + adresa + registrační a kontaktní údaje vpravo). „Vybrat, ne tvořit."
    async _applyHFFromProfile() {
        let p = null;
        try { p = await this.readLawyerProfile(); } catch (e) {}
        if (!window.LexisLetterhead || !window.LexisLetterhead.hasContent(p)) {
            this.customAlert('Nejdřív vyplň <b>Profil kanceláře</b> (Nastavení → Profil / hlavičkový papír), pak se hlavička složí sama.');
            return;
        }
        const logo = (window.LexisLetterhead.safeLogo(p.logo)) || (window.LEXIS_FIRM_LOGO || '');
        if (logo) {
            this._hfImages['left'] = logo;
            const imgEl = document.getElementById('hf-img-left');
        ...
        const c = [];
        if (p.tel) c.push('tel. ' + p.tel);
        if (p.email) c.push(p.email);
        if (p.web) c.push(p.web);
        if (c.length) lines.push(c.join(' · '));
        setV('hf-right', lines.join('\n'));
        this.switchHFTab('layout');
        this.updateHFPreview();
    },
